...
func _move_to_attack(chain: AI_Chain) -> void:
	...
	match index:
		...
		1:
			...
			_chain.add(_actioner, 'attack_combo_by_name', ['JumpSmash', funcref(_awareness, 'target'), ['down_move']])
		2:
		...
		3:
			...
			_chain.add(_actioner, 'attack_combo_by_name', ['JumpAttack', funcref(_awareness, 'target'), ['up_move']])
